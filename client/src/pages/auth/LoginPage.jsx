    ...
    <AuthSplitLayout
      ...
      footer={
        <div className="text-center text-sm text-[var(--text-muted)]">
          New here? <Link to="/register" className="font-semibold text-primary-500">Create account</Link>
        </div>
      }
    >
      ...
      {twoFactorChallenge ? (
        <div className="space-y-4">
          <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-soft)] p-3 text-sm text-[var(--text-muted)]">
            ...
            {' '}
            <span className="font-semibold text-[var(--text)]">{twoFactorChallenge.email}</span>
            . Enter authenticator app code (ya backup code).
          </div>

          <div>
            <p className="mb-2 text-sm font-semibold text-[var(--text)]">Authenticator / Backup Code</p>
            <div
                ...
                {Array.from({ length: 6 }).map((_, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center">
                      <span className="h-6 text-lg font-semibold tracking-wide text-[var(--text)]">{char || ' '}</span>
                      <span className={`mt-1 h-[2px] w-full rounded-full ${active ? 'bg-primary-500' : 'bg-[var(--border)]'}`} />
                    </div>
                  )
                })}
              </div>
            </div>
            <p className="mt-2 text-xs text-[var(--text-muted)]">6-digit authenticator ya 6-char backup code enter karo.</p>
          </div>

          <Button className="w-full justify-center" onClick={verifyTwoFactorLogin} disabled={twoFactorLoading}>
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex items-center justify-between gap-3 text-sm text-[var(--text-muted)]">
            ...
            <Link to="/forgot-password" className="font-semibold text-primary-500">Forgot password?</Link>
          </div>
